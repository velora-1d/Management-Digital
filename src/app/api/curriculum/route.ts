...
export async function GET(req: Request) {
  try {
    ...
    // Check if we have results to avoid querying with an empty array
    if (results.length === 0) {
      return NextResponse.json([]);
    }

    // Fetch components for all curriculums at once to prevent N+1 query bottleneck
    ...
    type GradeComponentType = typeof gradeComponents.$inferSelect;
    const componentsMap = new Map<number, GradeComponentType[]>();
