...
export async function GET(req: Request) {
  try {
    ...
    let detailedData = results.map(cur => ({ ...cur, gradeComponents: [] as GradeComponent[] }));
    if (results.length > 0) {
      ...
      const componentsMap = new Map<number, typeof allComponents>();
      for (const comp of allComponents) {
        if (comp.curriculumId !== null) {
          const existing = componentsMap.get(comp.curriculumId) || [];
          existing.push(comp);
          componentsMap.set(comp.curriculumId, existing);
        }
      }

      detailedData = results.map(cur => ({
        ...cur,
