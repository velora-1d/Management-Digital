...
export async function GET(req: Request) {
  try {
    ...
    // Optimized: Replace N+1 query with inArray batch fetch and memory Map
    const curriculumIds = results.map(cur => cur.id).filter((id): id is number => id !== null);
    ...
    const detailedData = results.map((cur) => {
        return {
            ...cur,
            gradeComponents: cur.id !== null ? (componentsMap.get(cur.id) || []) : []
        };
    });
