...
export async function GET(
  ...
  try {
    ...
    if (type === "infaq") {
      ...
      // Pre-fetch all payments for the bills
      ...
      for (const payment of allPayments) {
        if (payment.billId) {
          paymentsMap.set(payment.billId, (paymentsMap.get(payment.billId) || 0) + payment.amountPaid);
        }
      }
    ...
    if (type === "tabungan") {
      ...
      const allSavingsData = studentIds.length > 0
        ? await db
      ...
      for (const sv of allSavingsData) {
        if (sv.studentId) {
          const currentBalance = balancesMap.get(sv.studentId) || 0;
          const change = sv.type === "setor" ? sv.amount : (sv.type === "tarik" ? -sv.amount : 0);
          balancesMap.set(sv.studentId, currentBalance + change);
        }
      }
