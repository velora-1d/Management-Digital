...
export async function GET(
  ...
  try {
    ...
    if (type === "infaq") {
      ...
      const paymentsByBillId = new Map<number, number>();
      for (const p of allPayments) {
        if (p.billId !== null) {
          paymentsByBillId.set(p.billId, (paymentsByBillId.get(p.billId) || 0) + p.amountPaid);
        }
      }

      const result = bills.map((bill) => {
        const paid = paymentsByBillId.get(bill.id) || 0;
    ...
    if (type === "tabungan") {
      ...
      const allSavings = studentIds.length > 0
        ? await db
      ...
      const balancesByStudentId = new Map<number, number>();
      for (const sv of allSavings) {
        if (sv.studentId !== null) {
          const currentBalance = balancesByStudentId.get(sv.studentId) || 0;
          if (sv.type === "setor") {
            balancesByStudentId.set(sv.studentId, currentBalance + sv.amount);
      ...
      for (const s of activeStudents) {
          const balance = balancesByStudentId.get(s.id) || 0;
          if (balance !== 0) {
              result.push({
                student_id: s.id,
                student_name: s.name,
