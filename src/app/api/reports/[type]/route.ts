...
export async function GET(
  ...
  try {
    ...
    if (type === "infaq") {
      ...
      // Optimization: Batch fetch payments to prevent N+1 query bottleneck
      ...
      if (billIds.length > 0) {
        ...
        allPayments.forEach(p => {
          if (p.billId) {
            ...
            paymentsMap.set(p.billId, current + (p.amountPaid || 0));
          }
        });
      }
    ...
    if (type === "tabungan") {
      ...
      // Optimization: Batch fetch savings to prevent N+1 query bottleneck
      ...
      if (studentIds.length > 0) {
        ...
        allSavingsData.forEach(sv => {
          if (sv.studentId) {
            ...
            if (sv.type === "setor") {
              balancesMap.set(sv.studentId, currentBalance + (sv.amount || 0));
