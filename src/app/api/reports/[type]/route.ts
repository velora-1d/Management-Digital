...
export async function GET(
  ...
  try {
    ...
    if (type === "infaq") {
      ...
      let allPayments: { billId: number; amountPaid: number }[] = [];
      if (bills.length > 0) {
      ...
      for (const p of allPayments) {
        const currentPaid = paymentsMap.get(p.billId) || 0;
        paymentsMap.set(p.billId, currentPaid + Number(p.amountPaid));
      }
    ...
    if (type === "tabungan") {
      ...
      const result = [];
      let allSavings: { studentId: number; type: string; amount: number }[] = [];
      ...
      for (const sv of allSavings) {
        ...
        if (sv.type === "setor") {
          savingsMap.set(sv.studentId, currentBalance + amount);
