...
/**
 * GET /api/ppdb/export — Export data PPDB ke CSV
 * Query: ?status=
 */
export async function GET(request: Request) {
  try {
    ...
    const rows = list.map((r, i) => ({
      no: i + 1,
      no_form: r.formNo || "-",
      nama: (r.name || "").replace(/"/g, '""'),
      jenis_kelamin: r.gender === "L" ? "Laki-laki" : "Perempuan",
      tempat_lahir: r.birthPlace || "-",
      tanggal_lahir: r.birthDate || "-",
      nisn: r.nisn || "-",
      nik: r.nik || "-",
      ayah: (r.fatherName || "").replace(/"/g, '""'),
      ibu: (r.motherName || "").replace(/"/g, '""'),
      telepon: r.phone || "-",
      alamat: (r.address || "").replace(/"/g, '""'),
      asal_sekolah: (r.previousSchool || "").replace(/"/g, '""'),
      kelas_tujuan: r.targetClassroom || "-",
      status: r.status,
    }));
    ...
    return new Response(csv, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="ppdb_${new Date().toISOString().split("T")[0]}.csv"`,
      },
    });
  } catch (error) {
