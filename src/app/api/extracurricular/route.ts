...
export async function GET(req: Request) {
  try {
    ...
    const extIds = results.map((r) => r.id);
    let allMembers: {
      id: number;
    ...
    if (extIds.length > 0) {
      allMembers = await db
          ...
          .where(inArray(extracurricularMembers.extracurricularId, extIds));
    }

    const membersByExt = new Map<number, { id: number; student: typeof allMembers[0]['student'] }[]>();
    for (const member of allMembers) {
        if (member.extracurricularId === null) continue;
        if (!membersByExt.has(member.extracurricularId)) {
            membersByExt.set(member.extracurricularId, []);
        }
        membersByExt.get(member.extracurricularId)!.push({
            id: member.id,
    ...
    const detailedData = results.map((item) => ({
        ...
        members: membersByExt.get(item.id) || []
    }));
