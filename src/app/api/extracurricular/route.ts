...
export async function GET(req: Request) {
  try {
    ...
    // ⚡ Bolt: Fix N+1 query by pre-fetching all members in a single query
    ...
    const membersByExtracurricularId = new Map<number, typeof allMembers>();
    for (const member of allMembers) {
      if (member.extracurricularId === null) continue;
      if (!membersByExtracurricularId.has(member.extracurricularId)) {
        membersByExtracurricularId.set(member.extracurricularId, []);
      }
      membersByExtracurricularId.get(member.extracurricularId)!.push({
        ...member
      });
    }

    const detailedData = results.map((item) => {
        const members = membersByExtracurricularId.get(item.id) || [];
        // Map members to the exact structure expected by removing the internal extracurricularId key
