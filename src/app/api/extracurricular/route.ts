...
export async function GET(req: Request) {
  try {
    ...
    // ⚡ Bolt: Prevent N+1 queries by fetching all members for the current page in a single query
    const itemIds = results.map((item) => item.id);
    let allMembers: {
      id: number;
      extracurricularId: number;
      student: { id: number; name: string; nisn: string; } | null;
    }[] = [];
    ...
    // ⚡ Bolt: Group members by extracurricularId using an O(n) Map for fast lookups
    ...
    const detailedData = results.map((item) => {
      const members = (membersMap.get(item.id) || []).map(m => ({
        id: m.id,
        student: m.student
      }));
