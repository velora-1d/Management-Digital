...
export async function GET(req: Request) {
  try {
    ...
    // ⚡ Bolt Optimization: Replace N+1 query map loop with a single batch query
    const extracurricularIds = results.map(item => item.id);
    type MemberData = { id: number; student: { id: number | null; name: string | null; nisn: string | null } | null };
    const membersMap = new Map<number, MemberData[]>();
    ...
    if (extracurricularIds.length > 0) {
      ...
      for (const member of allMembers) {
        if (!member.extracurricularId) continue;
        const membersList = membersMap.get(member.extracurricularId) || [];
        membersList.push({ id: member.id, student: member.student });
