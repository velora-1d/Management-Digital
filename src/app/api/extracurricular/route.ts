...
export async function GET(req: Request) {
  try {
    ...
    // Extract IDs to batch fetch members
    const extracurricularIds = results.map(item => item.id);
    let allMembers: {
      extracurricularId: number | null,
      id: number,
      student: { id: number | null, name: string | null, nisn: string | null } | null
    }[] = [];
    ...
    const detailedData = results.map(item => {
        // Exclude extracurricularId from the member object to match original shape
        const members = (membersMap.get(item.id) || []).map(({ extracurricularId, ...rest }) => rest);
        return {
            ...item,
