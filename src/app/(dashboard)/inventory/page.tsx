    ...
    try {
      ...
      Swal.fire({
        ...
        didOpen: () => {
          ...
          (document.getElementById("swal-inv-qty") as HTMLInputElement).value = (item.quantity || 1).toString();
          ...
          (document.getElementById("swal-inv-cost") as HTMLInputElement).value = (item.acquisitionCost || 0).toString();
        },
        showCancelButton: true,
        confirmButtonText: "Simpan",
        cancelButtonText: "Batal",
              ...
              {isLoading ? (
                ...
                data.map((item, i) => {
                  ...
                  if (item.condition === "Baik") {
                    BadgeComponent = <span className="px-3 py-1 bg-emerald-50 text-emerald-700 font-bold text-xs rounded-full border border-emerald-100">Baik</span>;
                  } else if (item.condition === "Rusak Ringan") {
                    BadgeComponent = <span className="px-3 py-1 bg-amber-50 text-amber-700 font-bold text-xs rounded-full border border-amber-100">Rusak Ringan</span>;
                  } else {
                    BadgeComponent = <span className="px-3 py-1 bg-red-50 text-red-700 font-bold text-xs rounded-full border border-red-100">Rusak Berat</span>;
                  }
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors" style={{ borderBottom: "1px solid #f1f5f9" }}>
                      ...
                      <td style={{ padding: "1.25rem 1.5rem", textAlign: "center", verticalAlign: "middle" }}>{BadgeComponent}</td>
