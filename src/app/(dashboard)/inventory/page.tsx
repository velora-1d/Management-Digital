  ...
  const handleEdit = async (id: number) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        `,
        didOpen: () => {
          const nameInput = document.getElementById("swal-inv-name") as HTMLInputElement;
          const catInput = document.getElementById("swal-inv-cat") as HTMLInputElement;
          const qtyInput = document.getElementById("swal-inv-qty") as HTMLInputElement;
          const condInput = document.getElementById("swal-inv-cond") as HTMLSelectElement;
          const locInput = document.getElementById("swal-inv-loc") as HTMLInputElement;
          const costInput = document.getElementById("swal-inv-cost") as HTMLInputElement;

          if (nameInput) nameInput.value = item.name || "";
          if (catInput) catInput.value = item.category || "";
          if (qtyInput) qtyInput.value = item.quantity ? String(item.quantity) : "1";
          if (condInput) condInput.value = item.condition || "Baik";
          if (locInput) locInput.value = item.location || "";
          if (costInput) costInput.value = item.acquisitionCost ? String(item.acquisitionCost) : "0";
        },
        showCancelButton: true,
        confirmButtonText: "Simpan",
        cancelButtonText: "Batal",
