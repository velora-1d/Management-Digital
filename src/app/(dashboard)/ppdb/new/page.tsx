...
export default function PpdbNewPage() {
  ...
  const Field = ({ label, name, type = "text", required = false, span2 = false, placeholder = "", children }: any) => (
    <div style={span2 ? { gridColumn: "span 2" } : {}}>
      ...
      {children || <input type={type} name={name} value={(f as any)[name]} onChange={handleChange} required={required} placeholder={placeholder} style={inputStyle} className="focus:border-sky-500 transition-colors" />}
    </div>
  );
  ...
  const Select = ({ label, name, required = false, options }: { label: string; name: string; required?: boolean; options: [string, string][] }) => (
    <div>
      <label style={labelStyle}>{label} {required && <span style={{ color: "#e11d48" }}>*</span>}</label>
      <select name={name} value={(f as any)[name]} onChange={handleChange} required={required} style={inputStyle} className="focus:border-sky-500 bg-white">
